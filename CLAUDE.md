...
- **teams-cli boundary**: `teams(*args)` = read commands (append `--json`, parse the `{ok,data}` envelope). `teams_do(*args)` = mutating commands (NO `--json` — they reject it). For **mutating** commands, user text is passed after `--` so a leading `-` isn't parsed as a flag. **Read** commands can't use `--` (it would make the appended `--json` positional and break parsing), so user text going into a read command must instead reject a leading `-` (see `handle_search`).
